        ...
        <x-slot name="content">
            <div>
                <a href="{{ route('user.create') }}"
                    class="bg-bay-950 text-white rounded-lg px-3 py-2 text-xs border border-secondary-4 shadow-lg flex gap-1 items-center justify-center mb-12 md:mb-4 whitespace-nowrap w-min font-medium">
                    <svg viewBox="0 0 24 24" fill="none" class="w-3 h-3 stroke-white" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="rounded-lg overflow-hidden shadow-lg border border-secondary-4">
                        <table id="datatable" class="text-sm hover stripe row-border">
                            <thead class="bg-secondary-3 text-secondary-2 font-medium">
                                <tr>
                                    <td class="text-xs !text-center w-20">ID</td>
                                                ...
                                                <div>
                                                    <div>
                                                        <a href="{{ route('user.edit', $item->id) }}">
                                                            <svg class="w-5 h-5 text-bay-500" aria-hidden="true"
                        ...
                        @foreach ($data as $item)
                            <x-modal.confirm-delete :id="$item->id" :name="'Data'" :action="route('user.destroy', $item->id)" />
                        @endforeach
