    ...
    <div class="border border-gray-300 shadow-lg rounded-lg w-full">
        <div class="px-5 py-3 bg-gray-100 rounded-t-lg border-b border-gray-300">
            <div class="flex gap-1 items-center">
                ...
                <p class="text-sm">Data Referensi OPD/SKPD</p>
            </div>
        </div>
        <div class="bg-white px-5 py-5 rounded-b-lg">
            <div class="flex items-center justify-between mb-5">
                ...
                <form action="{{ route('skpd.index') }}" method="GET">
                    ...
                    <table id="datatable" class="text-sm hover stripe row-border">
                        <thead class="bg-bay-950 text-white font-bold">
                            <tr>
                                ...
                                <td class="text-xs !text-start">Kode SKPD</td>
                                <td class="text-xs !text-start">Uraian</td>
                                <td class="text-xs !text-start">Pimpinan</td>
                                <td class="text-xs !text-start">Bendahara</td>
                                <td class="text-xs !text-start">Operator</td>
                                <td class="text-xs !text-start">Kontak</td>
                                <td class="text-xs !text-center">User</td>
                            </tr>
                        </thead>
                        <tbody class="text-secondary-2">
                            @foreach ($data as $item)
                                <tr class="text-xs">
                                    <td class="w-10 h-full !align-top">
                                        <p class="text-center !align-top">{{ $data->firstItem() + $loop->index }}
                                        </p>
                                    </td>
                                    <td>
                                        <p class="text-start {{ strlen($item->kdskpd) <= 7 ? 'font-bold' : '' }}">
                                            {{ $item->kdskpd }}</p>
                                    </td>
                                    <td>
                                        <p class="text-start  {{ strlen($item->kdskpd) <= 7 ? 'font-bold' : '' }}">
                                    ...
                                    <td class="h-full">
                                        <div class="space-y-3 flex flex-col justify-start h-full">
                                            <p class="text-start  {{ strlen($item->kdskpd) <= 7 ? 'font-bold' : '' }}">
                                                {{ $item->pim_jab1 }}</p>
                                            <p class="text-start ">
                                                {{ $item->pim_nm }}</p>
