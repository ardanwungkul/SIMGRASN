    ...
    <div class="border border-gray-300 shadow-lg rounded-lg w-full">
        <div class="px-5 py-3 bg-gray-100 rounded-t-lg border-b border-gray-300">
            <div class="flex gap-1 items-center">
                ...
                <p class="text-sm">Data Referensi Fungsional</p>
            </div>
        </div>
        <div class="bg-white px-5 py-5 rounded-b-lg">
            <div class="flex items-center justify-between mb-5">
                ...
                <form action="{{ route('fungsional.index') }}" method="GET">
                    ...
                    <table id="datatable" class="text-sm hover stripe row-border">
                        <thead class="bg-bay-950 text-white font-bold">
                            <tr>
                                ...
                                <td class="text-xs !text-center">Kode Fungsional</td>
                                <td class="text-xs !text-center">Kode Golongan</td>
                                <td class="text-xs !text-start">Uraian</td>
                                <td class="text-xs !text-start">Dasar</td>
                                <td class="text-xs !text-start">Kelompok Fungsional</td>
                                <td class="text-xs !text-center">Tunjangan Struktural</td>
                            </tr>
                        </thead>
                        <tbody class="text-secondary-2">
                            @foreach ($data as $item)
                                <tr class="text-xs">
                                    ...
                                    <td>
                                        <p class="text-center">{{ $item->kdfung }}</p>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <p class="text-start">{{ $item->dasar }}</p>
                                    </td>
                                    <td>
                                        <p class="text-start">{{ $item->kelompok ? $item->kelompok->uraian : '' }}</p>
                                    </td>
                                    <td>
                                        <p class="text-center">Rp. {{ number_format($item->rp_fung, 0, ',', '.') }}</p>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
